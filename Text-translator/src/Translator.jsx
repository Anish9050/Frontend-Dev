...
/**
 * Text Translator — React + Tailwind
 * Uses RapidAPI (text-translator2) to translate from English to a chosen language.
 *
 * Setup notes:
 * 1) Put your RapidAPI key in an env var named VITE_RAPIDAPI_KEY (Vite).
 *    Example: create a .env file with: VITE_RAPIDAPI_KEY=YOUR_KEY_HERE
 * 2) The API is called directly from the browser for simplicity. For production, prefer a tiny proxy server to hide your key.
 *
 * Fix: Removed `process.env` access which caused "ReferenceError: process is not defined" in the browser.
 *      We now exclusively use `import.meta.env.VITE_RAPIDAPI_KEY`.
 */
...
// Simple dev-time check: do we have a key? (does not reveal the key value)
